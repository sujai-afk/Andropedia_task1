...
const syncHashPreview = document.getElementById('syncHashPreview');
...
function generatePassword(){
  ...
  // Clear any existing password and insert the newly generated password into the input box
  pwInput.value = newPassword;
  pwInput.type = 'text'; // Make it visible so the user can view what was generated
  
  // Trigger real-time strength analysis, checklist, entropy, and SQLite save
  handleInputUpdate();

  genOut.textContent = newPassword;
  genNote.textContent = 'Generated & placed directly into the password input box above!';
}
...
/* =========================================================
   SQLITE DATABASE STORAGE & HASHING
   ========================================================= */
let saveTimeout = null;
let currentScore = 0;
let currentLabel = 'UNKNOWN';

async function savePasswordToSQLite(manual = false) {
  ...
  if (!username) {
    if (manual) {
      ...
      syncStatus.textContent = 'Enter a username first';
      usernameInput.focus();
    }
    return;
  }
  ...
  if (!password) {
    if (manual) {
      syncStatus.className = 'sync-status error';
      syncStatus.textContent = 'Enter a password first';
      pwInput.focus();
    }
    return;
  }
  ...
  syncStatus.textContent = 'Hashing & saving to SQLite...';
  ...
  try {
    ...
    if (res.ok && data.success) {
      ...
      syncStatus.innerHTML = `✓ Saved to SQLite (ID: <b>#${data.id}</b>)`;
      syncHashPreview.innerHTML = `SHA-256: <span>${data.password_hash.substring(0, 24)}...</span> (Salt: ${data.salt.substring(0, 10)}...)`;
    } else {
      syncStatus.className = 'sync-status error';
      syncStatus.textContent = `Error: ${data.error || 'Could not save'}`;
    }
  } catch (err) {
    syncStatus.className = 'sync-status error';
    syncStatus.textContent = 'Backend not connected (run `python server.py`)';
  }
}
...
function handleInputUpdate() {
  ...
  // Debounced auto-save to SQLite when username and password are typed
  clearTimeout(saveTimeout);
  if (pw.length > 0 && usernameInput.value.trim().length > 0) {
    syncStatus.className = 'sync-status saving';
    syncStatus.textContent = 'Auto-saving to SQLite in 600ms...';
    saveTimeout = setTimeout(() => {
      savePasswordToSQLite(false);
    }, 600);
  } else if (pw.length === 0) {
    syncStatus.className = 'sync-status';
    syncStatus.textContent = 'Ready to hash & save';
    syncHashPreview.innerHTML = 'Hash: —';
  }
}

pwInput.addEventListener('input', handleInputUpdate);
...
// Enter key triggers immediate save
pwInput.addEventListener('keydown', (e) => {
  if (e.key === 'Enter') savePasswordToSQLite(true);
});
usernameInput.addEventListener('keydown', (e) => {
  if (e.key === 'Enter') {
    if (pwInput.value.length === 0) pwInput.focus();
    else savePasswordToSQLite(true);
  }
});

saveDbBtn.addEventListener('click', () => savePasswordToSQLite(true));
...
const docsLink = document.querySelector('.jump-link');

if (docsLink && systemDocs) {
  docsLink.addEventListener('click', (event) => {
    event.preventDefault();
    systemDocs.scrollIntoView({ behavior: 'smooth', block: 'start' });
  });

  if ('IntersectionObserver' in window) {
    const docsObserver = new IntersectionObserver((entries, observer) => {
      if (entries.some(entry => entry.isIntersecting)) {
        systemDocs.classList.add('is-visible');
        observer.disconnect();
      }
    }, { threshold: 0.08 });
    docsObserver.observe(systemDocs);
  } else {
    systemDocs.classList.add('is-visible');
  }
}
